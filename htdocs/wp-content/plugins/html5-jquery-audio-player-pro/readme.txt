...
This is Pro version of the plugin which has the following features:
...
*	Advanced customisation control - adjust player width, choose whether to display ratings/artwork and more
...
1. First de-activate Lite (free) version of the plugin if you already have it installed
1. Upload the `html5-jquery-audio-pro` folder to the `/wp-content/plugins/` directory
...
1. Add player to any post/page using playlist shortcode
...
= 2.0 =
* Fixed jQuery issue causing a conflict in some sites
* Fixed drag and drop issue that was affecting some users
* Added auto-play on/off option in display settings
* Added back in the functionality for user to set number of tracks to display before 'view more' button
...
* Added a widget so the player can be used in a sidebar
* Re-structured the backend admin so songs are now grouped under their parent playlist. This will be an improvement for users with large number of playlists

= 1.8 =
* More fixes for WordPress Core 3.5
* Fixed issue with 'add media' button disappearing
* Fixed problem where player was preventing some widgets opening.

= 1.7 =
* Fixed a bug where the playlist dropdown was disapearing for some users

= 1.6 =
* Implemented fixes so the plugin is now compatible with WordPress Core 3.5

= 1.5 =
* CSS issue fixed. Odd characters/symbols issue fix. 

= 1.4 =
* Minor fixes and enhancements. Bug fix for site url/wp install folder. More field character limits fixed. Made input fields longer. Created on/off ratings button. 

= 1.3 =
* Major Update. Added major new features including drag n drop playlist manager, greater customisation control, multiple buy/download buttons option. Also fixed some minor bugs and general issues such as URL truncating.

= 1.2 =
* Fixed a bunch of small issues to improve the plugin including table prefix bug fixed, character/symbol limitations fixed, display issue fixed

= 1.1 =
* Fixed jQuery/AJAX bug which was preventing playlist dropdown selection for some users
