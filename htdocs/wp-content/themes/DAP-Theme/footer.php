<div class="bottom-background">
	<div class="wrap">
		<footer>
			<div class="floated-footers group">
				<div id="footer_left_side">
					
						<h4>David Andersen Pianos is located</h4>
						
						<p>	on the West side of Los Angeles, CA
						<br />
						Call to make an appointment to visit the showroom  <br /> 
							<a href="tel:310-391-4360">(310) 391-4360</a>
						</p>
				</div>
				
				<div id="footer_right_side">
					<h4>Ask us how:</h4>
					<p>
						Artisanal, custom-built, WAY-better-than-new Steinway for 40% less.
					</p>
				</div>
			</div>
			
			<div id="copyright"><p>&copy;<?php echo date("Y"); echo " "; bloginfo('name'); ?></p></div>
		 	
		 	<div id="ajd_colophon"><p>Designed and built by -<a href="http://www.artichokejalapenodesign.com"> Artichoke Jalapeno Design</a></p></div>
				
			
		</footer>
	</div>
</div> 

			

	</div>

	<?php wp_footer(); ?>
	
	<!-- Don't forget analytics -->
	
</body>

</html>
